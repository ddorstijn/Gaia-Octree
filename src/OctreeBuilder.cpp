...
// OctreeBuilder constructor: this initializes the builder and sets up the
// output files, ready to go
OctreeBuilder::OctreeBuilder(std::string base_filename, size_t gridlength,
    ...
    , base_filename(base_filename)
{
    ...
    writeVoxelData(data_out, VoxelData(),
                   b_data_pos); // first data point is NULL

    // We store a simple white voxel in
    // case of Binary voxelization
    // all leafs will refer to this
    VoxelData v = VoxelData(0, vec3(), vec3(1.0, 1.0, 1.0));
    writeVoxelData(data_out, v, b_data_pos);
}

// Finalize the tree: add rest of empty nodes, make sure root node is on top
void
OctreeBuilder::finalizeTree()
{
    // fill octree
    ...
    // write header
    OctreeInfo octree_info(1, base_filename, gridlength, b_node_pos,
                           b_data_pos);
...
Node
OctreeBuilder::groupNodes(const vector<Node>& buffer)
{
    Node parent = Node();
    ...
    for (int k = 0; k < 8; k++) {
        if (!buffer[k].isNull()) {
            if (first_stored_child) {
                parent.children_base =
                    writeNode(node_out, buffer[k], b_node_pos);
                parent.children_offset[k] = 0;
                first_stored_child = false;
            } else {
                parent.children_offset[k] =
                    (char)(writeNode(node_out, buffer[k], b_node_pos) -
                           parent.children_base);
            }
        } else {
...
// Add an empty datapoint at a certain buffer level, and refine upwards from
// there
void
OctreeBuilder::addEmptyVoxel(const int buffer)
{
    b_buffers[buffer].push_back(Node());
    refineBuffers(buffer);
    b_current_morton = static_cast<::uint64_t>(
        b_current_morton +
        pow(8.0,
            b_maxdepth - buffer)); // because we're adding at a certain level
}

// REFINE BUFFERS: check all levels from start_depth up and group 8 nodes on a
// higher level
void
OctreeBuilder::refineBuffers(const int start_depth)
{
    for (int d = start_depth; d >= 0; d--) {
        if (b_buffers[d].size() == 8) { // if we have 8 nodes
            assert(d - 1 >= 0);
            if (isBufferEmpty(b_buffers[d])) {
                b_buffers[d - 1].push_back(
                    Node()); // push back NULL node to represent 8 empty nodes
            } else {
                b_buffers[d - 1].push_back(
                    groupNodes(b_buffers[d])); // push back parent node
            }
...
// Add a datapoint to the octree: this is the main method used to push
// datapoints
void
OctreeBuilder::addVoxel(const ::uint64_t morton_number)
{
...
// datapoints
void
OctreeBuilder::addVoxel(const VoxelData& data)
{
    // Padding for missed morton numbers
